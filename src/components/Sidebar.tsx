...
const navItems = [
  ...
  { icon: Target, label: "Goals", href: "/dashboard/goals", active: false },
  { icon: Activity, label: "Analytics", href: "/dashboard/analytics", active: false },
];
        ...
        <nav className="flex-1 space-y-2 px-4 py-8">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <Icon className={cn("h-5 w-5", item.active ? "text-primary" : "text-white/40 group-hover:text-white/70")} />
                {item.label}
              </Link>
            );
          })}
